...
const Sudoku: React.FC<{ question: string; questionId: string }> = ({
  ...
  const savePuzzleData = ():void => {
    ...
    savedData.wrongIdx = [...wrongIdx];

    console.log(savedData);
  }

  const loadPuzzleData = ():void => {
    setPuzzleData([...savedData.puzzleData]);
    setWrongIdx([...savedData.wrongIdx]);
  }
  ...
  const checkIsDone = (newData: number[]) => {
    ...
    const checkSqr = (startIdx: number): void => {
      ...
      for (let i = 0; i < 9; i++) {
        ...
        if (number > 0 && indexof !== i) {
          wrong
            .add(startIdx + (Math.floor(i / 3) * 9 + (i % 3)))
            .add(startIdx + (Math.floor(indexof / 3) * 9 + (indexof % 3)));
        }
      }
    };
    ...
    for (let checkIdx of checkIdxArr) {
      ...
      const sqrStartIdx: number =
        Math.floor(row / 3) * 27 + Math.floor(col / 3) * 3;
      checkRowSet.add(row);
      checkColSet.add(col);
    ...
    const newWrongIdx = Array.from(wrong.values());
  ...
  return (
    <>
      ...
      />
      {watchIsRunning && <OptionalButton savePuzzleData={savePuzzleData} loadPuzzleData={loadPuzzleData} />}
      {!watchIsRunning && (
